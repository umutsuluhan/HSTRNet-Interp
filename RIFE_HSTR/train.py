...
from model.RIFE_HSTR.RIFE.RIFE import Model
from model.RIFE_HSTR.RIFE.warplayer import warp
from dataset import *
...
def train(model):
    ...
    dataset_HR = VimeoDataset('train',args.data_root, HR = True)
    train_data_HR = DataLoader(dataset_HR, batch_size=args.batch_size, num_workers=0, drop_last=True)
    
    dataset_LR = VimeoDataset('train',args.data_root, HR=True)
    train_data_LR = DataLoader(dataset_LR, batch_size=args.batch_size, num_workers=0, drop_last=True)
    
    args.step_per_epoch = train_data_HR.__len__()
    
    dataset_val_HR = VimeoDataset('validation', args.data_root, HR=True)
    val_data_HR = DataLoader(dataset_val_HR, batch_size=12, num_workers=0)
    
    dataset_val_LR = VimeoDataset('validation', args.data_root, HR=True)
    val_data_LR = DataLoader(dataset_val_LR, batch_size=12, num_workers=0)
    
    
    # evaluate(model, val_data_HR, val_data_LR, nr_eval, writer_val)
    ...
    for epoch in range(args.epoch):
        for i, (data_HR, data_LR) in enumerate(zip(train_data_HR, train_data_LR)):
            ...
            pred, merged_img, flow_HR, flow_LR, loss_l1, loss_flow, loss_ter = model.update(imgs, gt, learning_rate, mul, True)
            ...
            if step % 1000 == 1:
                ...
                flow_HR = flow_HR.permute(0, 2, 3, 1).detach().cpu().numpy()
                flow_LR = flow_LR.permute(0, 2, 3, 1).detach().cpu().numpy()
                for i in range(5):
                    imgs = np.concatenate((merged_img[i], pred[i], gt[i]), 1)[:, :, ::-1]
                    writer.add_image(str(i) + '/img', imgs, step, dataformats='HWC')
                    writer.add_image(str(i) + '/flow_HR', flow2rgb(flow_HR[i]), step, dataformats='HWC')
                    writer.add_image(str(i) + '/flow_LR', flow2rgb(flow_LR[i]), step, dataformats='HWC')
                writer.flush()
            print('epoch:{} {}/{} time:{:.2f}+{:.2f} loss_l1:{:.4e}'.format(epoch, i, args.step_per_epoch, data_time_interval, train_time_interval, loss_l1))
            step += 1
        nr_eval += 1
        if nr_eval % 5 == 0:
            evaluate(model, val_data_HR, val_data_LR, step, writer_val)
        model.save_model(log_path)    
