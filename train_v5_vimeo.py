...
def train(model):
    ...
    model.ifnet.load_state_dict(
        convert(torch.load('/home/mughees/Projects/HSTR_ICIP/model/RIFE_v5/train_log/flownet.pkl', map_location=device)))
    model.ifnet.eval()
    
    model.contextnet.load_state_dict(torch.load('./model_dict/HSTR_contextnet_150.pkl', map_location=device))

    model.unet.load_state_dict(torch.load('./model_dict/HSTR_unet_150.pkl', map_location=device))
    
    optimizer.load_state_dict(torch.load('./model_dict/HSTR_optimizer150.pkl', map_location=device))
