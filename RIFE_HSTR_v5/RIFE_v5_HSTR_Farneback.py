...
class FusionNet(nn.Module):
    ...
    def forward(self, img0_HR, img1_HR, img0_LR, img2_LR, img1_LR, flow_HR, flow_LR_1_0, flow_LR_1_2, c0_HR, c1_HR, c0_LR, c1_LR):
        warped_img0_HR = warp(img0_HR, flow_HR[:, :2])
        warped_img1_HR = warp(img1_HR, flow_HR[:, 2:4])
        warped_img0_LR = warp(img0_LR, flow_LR_1_0)
        warped_img2_LR = warp(img2_LR, flow_LR_1_2)
...
class Model:
    ...
    def predict(self, imgs, flow_HR, flow_LR_1_0, flow_LR_1_2, training=True):
        img0_HR = imgs[:, :3]
        img1_HR = imgs[:, 3:6]
        img0_LR = imgs[:, 6:9]
        img1_LR = imgs[:, 9:12]
        ...
        c0_LR = self.contextnet(img0_LR, flow_LR_1_0)  #  F 1 --> 0
        c1_LR = self.contextnet(img2_LR, flow_LR_1_2)  #  F 1 --> 2
        
        for i in range(len(c0_LR)): 
            c0_LR[i]= F.interpolate(c0_LR[i], scale_factor=0.5, mode="bilinear", align_corners=False)         
            c1_LR[i] = F.interpolate(c1_LR[i], scale_factor=0.5, mode="bilinear", align_corners=False)  
        ...
        refine_output, warped_img0_HR, warped_img1_HR, warped_img0_LR, warped_img2_LR = self.fusionnet(
            img0_HR, img1_HR, img0_LR, img1_LR, img2_LR, flow_HR, flow_LR_1_0, flow_LR_1_2, c0_HR, c1_HR, c0_LR, c1_LR)
    ...
    def inference(self, imgs, scale=None):
        ...
        flow_LR_1_0 = self.calculate_flow(               # Flow from t=0 to t=1 (low sr, high fps video)
            torch.cat((img1_LR, img0_LR), 1))
       
        flow_LR_1_2 = self.calculate_flow(               # Flow from t=2 to t=1 (low sr, high fps video)
            torch.cat((img1_LR, img2_LR), 1))
