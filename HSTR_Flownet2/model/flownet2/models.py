...
class FlowNet2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        resampled_img1 = self.resample1(torch.cat((x[:,3:,:,:], flownetc_flow), 1))
        ...
        resampled_img1 = self.resample2(torch.cat((x[:,3:,:,:], flownets1_flow), 1))
        diff_img0 = x[:,:3,:,:] - resampled_img1
        norm_diff_img0 = self.channelnorm(diff_img0)
        ...
        diff_flownets2_flow = self.resample4(torch.cat((x[:,3:,:,:], flownets2_flow), 1))
        ...
        diff_flownetsd_flow = self.resample3(torch.cat((x[:,3:,:,:], flownetsd_flow), 1))
